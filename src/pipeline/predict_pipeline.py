...
class PredictPipeline:
    ...
    def save_input_files(self):
        try:
            ...
            input_csv_file=self.requsest.files('file')
    ...
    def get_predicted_dataframe(self,input_dataframe_path:pd.DataFrame):
        try:
            prediction_columnn_name:str=TARGET_COLUMN
            input_dataframe:pd.DataFrame=pd.read_csv(input_dataframe_path)

            # Drop the target column from the input dataframe
            input_dataframe=input_dataframe.drop(columns="Unnamed: 0") if 'Unnamed: 0' in input_dataframe.columns else input_dataframe

            # Create prediction output directory if it doesn't exist
            predictions=self.predict(input_dataframe)

            # Create prediction output directory if it doesn't exist
            input_dataframe[prediction_columnn_name]=[pred for pred in predictions]

            target_column_mapping={0:'bad',1:'good'}

            input_dataframe[prediction_columnn_name]=input_dataframe[prediction_columnn_name].map(target_column_mapping)
            ...
            logging.info("Predicted completed")
        
        except Exception as e:
            raise CustomException(e,sys) from e
        
    # Method to run the prediction pipeline
